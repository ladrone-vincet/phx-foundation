defmodule Foundation.Videos.YoutubeData do
  import Phoenix.Controller, only: [put_flash: 3, redirect: 2]
  import FoundationWeb.Router.Helpers
  ...
  def create_or_show_video(conn, regex) do
    ...
    case Foundation.Repo.insert(changeset) do
      {:ok, video} ->
        conn
          |> put_flash(:info, "Video created successfully.")
          |> redirect(to: video_path(conn, :show, video))
      {:error, _video} ->
        video = Video |> Foundation.Repo.get_by(video_id: video_id)
        conn
          |> put_flash(:info, "Video has alredy been created.")
          |> redirect(to: video_path(conn, :show, video))
    end
  end
  ...
  defp get_formatted_time(video_data) do
    ...
    [hours, minutes, seconds] =
      for x <- duration, do: hd(Regex.run(~r{\d_}, x) || ["0"]) |> String.to_integer
